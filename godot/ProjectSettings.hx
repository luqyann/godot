...
Feature tags: Project settings can be overridden for specific platforms and configurations (debug, release, ...) using [$DOCS_URL/tutorials/export/feature_tags.html](feature tags).

Overriding: Any project setting can be overridden by creating a file named `override.cfg` in the project's root directory. This can also be used in exported projects by placing this file in the same directory as the project binary. Overriding will still take the base project settings' [$DOCS_URL/tutorials/export/feature_tags.html](feature tags) in account. Therefore, make sure to also override the setting with the desired feature tags if you want them to override base project settings on all platforms and configurations.
**/
@:libType
@:csNative
...
extern class ProjectSettings {
	...
	**/
	public static var onProjectSettingsChanged(get, never):Signal<Void->Void>;
	@:dox(hide) @:noCompletion inline static function get_onProjectSettingsChanged():Signal<Void->Void> {
		return new Signal(SINGLETON, "project_settings_changed", Signal.SignalHandlerVoidVoid.connectSignal, Signal.SignalHandlerVoidVoid.disconnectSignal, Signal.SignalHandlerVoidVoid.isSignalConnected);
	}
	...
	**/
	...
	/**		
		Returns the absolute, native OS path corresponding to the localized `path` (starting with `res://` or `user://`). The returned path will vary depending on the operating system and user preferences. See [$DOCS_URL/tutorials/io/data_paths.html](File paths in Godot projects) to see what those paths convert to. See also `godot.ProjectSettings.localizePath`.
